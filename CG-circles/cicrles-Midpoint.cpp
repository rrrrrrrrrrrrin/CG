...
/*
	...
	5. Got the value of pStep when y doesn't increase (before recalculation of p in a loop): p = p + pStep. If it does p will change accordingly: (x stays the same)
		p = p + pIncreased, where pIncreased = pNext - p(y-1) ( p(y-1) = x^2 + (y+0.5-1) - R^2. We subtract 1 from y because y already was incremented
		before the recalculation of p) = x^2 + 2x + y^2 + y - R^2 + 1 - (x^2 + (y - 0.5)^2 - R^2) =
